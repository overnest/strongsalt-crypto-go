...
// Version 1:
// -----------------------------------------------------------------------------------------------------------------------------------------------
// | version(4 bytes) | saltLen(4 bytes) | salt | iterLen(4 bytes) | iter(4 bytes) | memoryLen(4 bytes) | memory | threadsLen(4 bytes) | threads |
// -----------------------------------------------------------------------------------------------------------------------------------------------
...
func (_ *Argon2) Deserialize(data []byte) (KdfBase, error) {
	...
	case VERSION_ONE:
		...
		_ = buf.Next(4)
		var threads int32
		err = binary.Read(buf, binary.BigEndian, &threads)
		if err != nil {
			return nil, err
		}
...
func (k *Argon2) Serialize() ([]byte, error) {
	...
	case VERSION_ONE:
		...
		err = binary.Write(buf, binary.BigEndian, k.threads)
		if err != nil {
			return nil, err
		}
